...
def index(request, vendor = ''):
    token = ''
    url_parts = RequestGrabber(request)
    if request.GET.get('token'):
        token = request.GET.get('token')
        debug(token = token, debug = True)
    ...
    notify.send("VIS", "[index] request.session.items() = " + str(request.session.items()), "vis", "index")
    notify.send("VIS", str(request.session.items()), "vis", "index")
    notify.send("VIS", "[index] is_root = " + str(is_root), "vis", "index")
    ...
    if cc[0] and not isinstance(cc[0], bool):
        ...
        #with connection.cursor() as cursor:
            ...
            #data = cursor.fetchall()
            #sql_script = "EXEC tmsp_VIS_information_dashboard @vendor = '{}'".format(vendor)
        ...
        #with connection.cursor() as cursor:
            #sql_script = "EXEC TMSP_VIS_Rangking_Dashboard @type = 0"
            #cursor.execute("EXEC TMSP_VIS_Rangking_Dashboard @type = 0")
            #data_per_month_1 = check_connection(request, cursor, sql_script)
            #debug(data_per_month_1 = data_per_month_1, debug = True)

        #with connection.cursor() as cursor:
            #sql_script = "EXEC TMSP_VIS_Rangking_Dashboard @type = 1"
            #cursor.execute("EXEC TMSP_VIS_Rangking_Dashboard @type = 1")
            #data_per_month_2 = check_connection(request, cursor, sql_script)
            #debug(data_per_month_2 = data_per_month_2, debug = True)

        #with connection.cursor() as cursor:
            #sql_script = "TMSP_VIS_YTM_DASHBOARD @Vendor = '{}'".format(vendor)
            #cursor.execute("TMSP_VIS_YTM_DASHBOARD @Vendor = '{}'".format(vendor))
            #data_year_to_month = check_connection(request, cursor, sql_script)
        ...
        #with connection.cursor() as cursor:
            #sql_script = "TMSP_VIS_YTD_DASHBOARD @Vendor = '{}'".format(vendor)
            #cursor.execute("TMSP_VIS_YTD_DASHBOARD @Vendor = '{}'".format(vendor))
            #data_year_to_date = check_connection(request, cursor, sql_script)
        ...
        debug(data = data, debug = True)
        debug(token = token, debug = True)
        debug(request_get = request.GET, debug = True)
        debug(dir_request_get = dir(request.GET), debug = True)
        debug(request_get = request.GET.items, debug = True)
        debug(request_path = request.get_full_path(), debug = True)
        debug(request_get_full_path = request.get_full_path(), debug = True)
        debug(request_path = request.path, debug = True)
        debug(request_path_info = request.path_info, debug = True)
        debug(request_build_absolute_uri = request.build_absolute_uri(), debug = True)
        debug(request = request, debug = True)
        debug(url_parts = url_parts, debug = True)
        return render(request, 'index.html', data)

    #with connection.cursor() as cursor:
        #cursor.execute("EXEC tmsp_information_VIS_dashboard @vendor = '{}'".format(vendor))
        #sql_script = "EXEC tmsp_information_VIS_dashboard @vendor = '{}'".format(vendor)
        #data = cursor.fetchall()
        #data = check_connection(request, cursor, sql_script)
        #total_po = data[0][1] or 0
        #total_grpo = data[1][1] or 0
    ...
    # return render(request, 'index.html', data, context_instance=RequestContext(request), content_type="application/xhtml+xml")
    add_log(request, "index")
    return render(request, 'index.html', data)
...
def login(request, message = '', username = ''):
    # check_login(request)
    #request.session.clear()
    #request.COOKIES.clear()
